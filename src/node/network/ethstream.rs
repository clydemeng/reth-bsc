...
use reth_eth_wire_types::{NetworkPrimitives, RawCapabilityMessage, EthMessageID};
    ...
    N: NetworkPrimitives,
{
    ...
    pub fn decode_message(&self, bytes: BytesMut) -> Result<EthMessage<N>, EthStreamError> {
        ...
        let msg = match ProtocolMessage::decode_message(self.version, &mut bytes.as_ref()) {
            Ok(m) => m,
            Err(err) => {
                // retry tolerant decoders for the two BSC message IDs
                // First byte = message id (because upstream ethstream prefixes it).
                let msg_id = *bytes.get(0).unwrap_or(&0);
                if msg_id == EthMessageID::NewBlockHashes as u8 {
                    if let Some(nb) = crate::net::compat::new_block_hashes(&bytes[1..]) {
                        return Ok(EthMessage::NewBlockHashes(nb));
                    }
                }
                if msg_id == EthMessageID::NewBlock as u8 {
                    if let Some(nb) = crate::net::compat::new_block::<N>(&bytes[1..]) {
                        return Ok(EthMessage::NewBlock(nb));
                    }
                }
